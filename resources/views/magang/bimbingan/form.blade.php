        ...
        <div class="px-6 py-4 space-y-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div x-data="enhancedFilePreview()" class="space-y-3">
                    <x-input-label for="path_dokumentasi" value="Dokumentasi Bimbingan (Foto/PDF/Dokumen)" />
                    <div class="relative">
                        <input type="file" name="path_dokumentasi" id="path_dokumentasi" @change="handleFileSelect($event)" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" accept=".pdf,.jpg,.jpeg,.png,.doc,.docx">
                    </div>
                    ...
                    <div x-show="fileUrl || existingFile" class="mt-3">
                        ...
                        <!-- Existing File -->
                        @if (isset($logBimbingan) && $logBimbingan && $logBimbingan->path_dokumentasi)
                            <template x-if="!fileUrl">
                                <div class="border rounded-lg p-3 bg-blue-50 cursor-pointer hover:shadow-md transition-shadow" @click="openExistingFile('{{ asset('storage/' . $logBimbingan->path_dokumentasi) }}', '{{ pathinfo($logBimbingan->path_dokumentasi, PATHINFO_EXTENSION) }}')">
